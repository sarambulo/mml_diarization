...
def train_triplet_model(
    model, train_loader, optimizer, device, margin=0.5, num_epochs=20
):
    model.train()
    ...
    for epoch in range(num_epochs):
        ...
        print(
            f"Epoch {epoch+1}/{num_epochs}, Triplet Loss: {avg_loss:.6f}, LR: {current_lr:.6f}"
        )
        print(
...
def debug_audio_dataset(msd_dataset):
    ...
    total_samples = min(
        100, len(msd_dataset)
    )  # Check first 100 samples or all if fewer
...
def main():
    ...
    train_loader = DataLoader(
        audio_dataset, batch_size=64, shuffle=True, collate_fn=triplet_collate_fn
    )

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

    # Use the compact model with increased dropout
    model = CompactAudioEmbedding(input_dim=40, embedding_dim=256, dropout_rate=0.3).to(
        device
    )
