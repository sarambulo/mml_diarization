...
class AudioActiveSpeakerModel(nn.Module):
    def __init__(
        self, base_model, embedding_dim=256, num_classes=1
...
def train_with_diarization_loss(
    ...
    for epoch in range(num_epochs):
        ...
        print(
            f"Processed {batch_count} batches, Skipped {skipped_batches} empty batches"
        )
    ...
    if total_processed == 0:
        print(
            "WARNING: No data was processed during training! Check your dataset filtering."
        )
    else:
...
def main():
    data_path = (
        "../preprocessed"  # Root directory where video folders like 00001, 00002 exist
    )
    partition_file = (
        "../data_sample/few_train.rttm"  # Should list video IDs like: 00001, 00002...
    )

    msd_dataset = MSDWildChunks(
        data_path=data_path, partition_path=partition_file, subset=1.0
    )
    ...
    base_model = CompactAudioEmbedding(
        input_dim=40, embedding_dim=256, dropout_rate=0.3
    ).to(device)

    # Create the complete model with classifier
    model = AudioActiveSpeakerModel(
        base_model=base_model, embedding_dim=256, num_classes=1  # Binary classification
    ).to(device)
