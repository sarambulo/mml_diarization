...
class AudioActiveSpeakerModel(nn.Module):
    def __init__(
        self, base_model, embedding_dim=512, num_classes=1
    ):  # Using num_classes=1 for binary classification
        super().__init__()
    ...
    def forward(self, x):
        ...
        return (
            embedding,
            probs,
        )  # Squeeze to get scalar output for binary classification
...
class AudioTripletDatasetWithLabels(Dataset):
    ...
    def process_audio(self, audio_segment):
        ...
        if self.augment:
            ...
            time_mask_param = max(
                1, min(20, mel.shape[-1] - 1)
            )  # More aggressive time masking
            freq_mask_param = max(
                1, min(10, mel.shape[-2] - 1)
            )  # More aggressive freq masking
...
class CompactAudioEmbedding(nn.Module):
    def __init__(self, input_dim=40, embedding_dim=512, dropout_rate=0.3):
        super().__init__()
...
def validate_model(model, criterion, val_loader, device):
    ...
    for batch in val_loader:
        ...
        with torch.no_grad():
            ...
            anchor_preds = (anchor_probs > 0.5).int()
            # print(anchor_preds, labels)
...
def train_with_diarization_loss(
    model,
    train_loader,
    val_loader,
    optimizer,
    ...
    scheduler = optim.lr_scheduler.ReduceLROnPlateau(
        optimizer,
        factor=0.8,  # Reduce LR by half when plateauing
        patience=8,  # Wait 2 epochs before reducing
    )
    ...
    print(
        f"Using batch size of {train_loader.batch_size}, resulting in {total_batches} batches"
    )
    ...
    for epoch in range(num_epochs):
        ...
        print(
            f"Epoch {epoch+1}/{num_epochs}, Validation Loss: {val_loss:.6f}, Acc: {val_acc}, LR: {current_lr:.6f}"
        )
        ...
        if val_loss < best_loss:
            ...
            # Save best model
            torch.save(
                model.state_dict(), "model_checkpoints/audio/best_audio_model.pth"
            )
...
def main(msd_dataset, batch_size, epochs):
    os.makedirs("model_checkpoints/audio", exist_ok=True)
    ...
    train_size = int(0.8 * len(audio_dataset_full))
    val_size = len(audio_dataset_full) - train_size
    ...
    triplet_lambda = 0.3  # Weight for triplet loss
    bce_lambda = 0.7  # Weight for BCE loss
