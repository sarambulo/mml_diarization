...
class FaceLipEmbedding(nn.Module):
    def __init__(self, pretrained_path='lipnet_pretrained.pt', coord_input_dim=40, final_embedding_dim=512):
        super(FaceLipEmbedding, self).__init__()
        
        self.base_model = LipCoordNet()
        checkpoint = torch.load(pretrained_path)
        self.base_model.load_state_dict(checkpoint['model_state_dict'] if 'model_state_dict' in checkpoint else checkpoint)
        ...
        self.detection_head = nn.Sequential(
            nn.Linear(final_embedding_dim, 1),
            nn.Sigmoid()
        )
...
class VisualOnlyLipModel(torch.nn.Module):
   def __init__(self, embedding_dims, num_classes):
      ...
      self.classifier = torch.nn.sequential(
          torch.nn.Linear(embedding_dims, 1),
          nn.Sigmoid()
      )
...
def extract_lip_coordinates(bbox, predictor_path="shape_predictor_68_face_landmarks.dat"):
