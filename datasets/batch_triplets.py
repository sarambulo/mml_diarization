...
PREFIX = "preprocessed"
BATCH_SIZE = 5000
OUTPUT_PREFIX = "batched_triplets"  # Where to save output .npz files
NUM_BATCHES = -1
...
def upload_npz(bucket, key, visual_data, audio_data, labels):
    buffer = io.BytesIO()
    np.savez_compressed(
        buffer, visual_data=visual_data, audio_data=audio_data, is_speaking=labels
    )
    buffer.seek(0)
...
for video_id in tqdm(video_ids):
    ...
    response = s3.list_objects_v2(
        Bucket=BUCKET, Prefix=os.path.join(base_prefix, "visual_pairs")
    )
    for obj in response.get("Contents", []):
        ...
        if len(v_buf) >= BATCH_SIZE:
            ...
            upload_npz(
                BUCKET, out_key, np.stack(v_buf), np.stack(a_buf), np.array(l_buf)
            )
