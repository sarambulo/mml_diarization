...
class TestMSDWildBase():
   ...
   def test_item(self):
      msdwild = MSDWildBase(Path(DATA_PATH), 'many_val')
      video_stream, audio_stream, rttm, faces_bounding_boxes = next(iter(msdwild))
...
class TestMSDWildFrames():
   ...
   def test_item(self):
      msdwild = MSDWildFrames(Path(DATA_PATH), 'many_val')
      anchor, positive_pair, negative_pair = next(iter(msdwild))
      assert self.triple_element(anchor)
      assert self.triple_element(positive_pair)
      assert self.triple_element(negative_pair)
   ...
   def triplet_element(self, element):
      video_frame, audio_segment, label, face = element
