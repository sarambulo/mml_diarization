...
def create_rttm_file(video_id, speaker_ids, labels, timestamps, output_path):
    ...
    with open(output_path, "w") as f:
        for video_id, speaker_ids, labels, timestamps in zip(
            video_id, speaker_ids, labels, timestamps
        ):
            data = pd.DataFrame(columns=speaker_ids[0], data=labels)
            data["Timestamp"] = timestamps
            data = data.melt(
                id_vars=["Timestamp"], var_name="Speaker ID", value_name="Speaking"
            )
            ...
            for i in range(len(speaker_ids)):
                f.write(
                    f"SPEAKER {int(video_id):05d} 0 {start[i]:.6f} {duration[i]:.6f} <NA> <NA> {speaker_ids[i]} <NA> <NA>\n"
                )
    return
