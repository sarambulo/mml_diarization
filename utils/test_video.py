...
def test_extract_faces():
    ...
    faces = extract_faces(
        video_frames=video_frames, frame_ids=frame_ids, bounding_boxes=bounding_boxes
    )
    face_ids = [0, 1, 2]
    num_frames = [1, 1, 1]
...
def test_transform_video():
    ...
    video_frames = transform_video(
        video_frames=video_frames, height=H, width=W, scale=scale
    )
