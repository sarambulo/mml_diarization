...
class AudioOnlyTDNN(nn.Module):
    def __init__(self, input_dim=40, hidden_dim=512):
        ...
        self.tdnn1 = nn.Conv1d(
            input_dim, hidden_dim, kernel_size=5, dilation=1, padding="same"
        )
        self.bn1 = nn.BatchNorm1d(hidden_dim)

        self.tdnn2 = nn.Conv1d(
            hidden_dim, hidden_dim, kernel_size=5, dilation=2, padding="same"
        )
        self.bn2 = nn.BatchNorm1d(hidden_dim)

        self.tdnn3 = nn.Conv1d(
            hidden_dim, hidden_dim, kernel_size=5, dilation=3, padding="same"
        )
...
def main():
    ...
    train_loader = DataLoader(
        audio_dataset, batch_size=batch_size, shuffle=True, collate_fn=collate_fn
    )
